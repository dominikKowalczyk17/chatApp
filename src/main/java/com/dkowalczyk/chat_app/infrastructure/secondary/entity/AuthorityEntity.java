...
import com.dkowalczyk.chat_app.messaging.domain.user.aggreagate.AuthorityBuilder;
...
public class AuthorityEntity {
    ...
    public static Set<AuthorityEntity> from(Set<Authority> authorities) {
        return authorities
                .stream()
                .map(authority ->
                        com.dkowalczyk.chat_app.infrastructure.secondary.entity.AuthorityEntityBuilder.authorityEntity()
    ...
    public static Set<Authority> toDomain(Set<AuthorityEntity> authorityEntities) {
        return authorityEntities.stream()
                .map(authority -> AuthorityBuilder.authority().name(new AuthorityName(authority.name)).build()).collect(Collectors.toSet());
    }
